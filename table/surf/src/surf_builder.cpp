...
void SuRFBuilder::buildSparse(const std::vector<std::string>& keys) {
    ...
    insertValToSuffix(i, level);
    insertValToValue(i+1, level);
    }
}
...
void SuRFBuilder::buildSparse(const std::vector<std::string>& keys,
    ...
    for (position_t i = 0; i < keys.size(); i++) {
        ...
        if (i < keys.size() - 1)
            ...
            level = insertKeyBytesToTrieUntilUnique(keys[curpos], std::string(), level);
        insertValToSuffix(suffixs[i], level);
        insertValToValue(vals[i], level);
    }
}
...
level_t SuRFBuilder::insertKeyBytesToTrieUntilUnique(const std::string& key, const std::string& next_key, const level_t start_level) {
    ...
    */
    ...
    // new node.
    is_start_of_node = true;
    bool same_as_next = key.length() <= next_key.length() && isSameKey(key, next_key.substr(0, key.length()));
    //while (level < key.length() && level < next_key.length() && key[level] == next_key[level]) {
	while (level < key.length()) {
        if(level >= next_key.length() || key[level] != next_key[level]) {
            same_as_next = false;
        }
        insertKeyByte(key[level], level, is_start_of_node, is_term);
	    level++;
...
inline void SuRFBuilder::insertValToSuffix(const uint64_t val, const level_t level) {            
    if (level >= getTreeHeight())
    addLevel();
    assert(level - 1 < suffixes_.size());
    storeSuffix(level, val);
}

inline void SuRFBuilder::insertValToValue(const uint64_t val, const level_t level) {
    if (level >= getTreeHeight())
    addLevel();
    assert(level - 1 < values_.size());
    storeValue(level, val);
}
...
inline void SuRFBuilder::storeValue(const level_t level, const word_t val)
{
    position_t pos = value_counts_[level - 1] * kValueSize;
    assert(pos <= (values_[level - 1].size() * kWordSize));
    if (pos == (values_[level - 1].size() * kWordSize))
        values_[level - 1].push_back(0);
    position_t word_id = pos / kWordSize;
    position_t offset = pos % kWordSize;
    position_t word_remaining_len = kWordSize - offset;
    if (kValueSize <= word_remaining_len) {
        word_t shifted_value = val << (word_remaining_len - kValueSize);
        values_[level - 1][word_id] += shifted_value;
    } else {
        word_t value_left_part = val >> (kValueSize - word_remaining_len);
        values_[level - 1][word_id] += value_left_part;
        values_[level - 1].push_back(0);
        word_id++;
        word_t value_right_part = val << (kWordSize - (kValueSize - word_remaining_len));
        values_[level - 1][word_id] += value_right_part;
    }
    value_counts_[level - 1]++;
}
...
inline uint64_t SuRFBuilder::computeDenseMem(const level_t downto_level) const {
    ...
    mem += (value_counts_[level] * kValueSize / 8);
    }
    return mem;
}
...
void SuRFBuilder::addLevel() {
    ...
    values_.push_back(std::vector<word_t>());
