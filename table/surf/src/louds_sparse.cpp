...
LoudsSparse::LoudsSparse(const SuRFBuilder* builder) {
    ...
    values_ = new BitvectorSuffix(surf::kReal, 0, kValueSize,
                                  builder->getValues(),
                                  num_value_bits_per_level, start_level_, height_);
}
...
int LoudsSparse::Iter::compare(const std::string& key) const {
    ...
    int cmp = iter_key.compare(key_sparse_same_length);
    if (cmp != 0) 
	return cmp;
...
int LoudsSparse::Iter::getValue(word_t* val) const {
    position_t value_pos = trie_->getSuffixPos(pos_in_trie_[key_len_ - 1]);
