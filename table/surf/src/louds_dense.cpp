...
LoudsDense::LoudsDense(const SuRFBuilder* builder) {
    ...
    std::vector<position_t> num_value_bits_per_level;
    for (level_t level = 0; level < height_; level++) {
        num_value_bits_per_level.push_back(builder->getValueCounts()[level] * kValueSize);
    }
    values_ = new BitvectorSuffix(surf::kReal, 0, kValueSize, 
                                  builder->getValues(),
                                  num_value_bits_per_level, 0, height_);
}
...
bool LoudsDense::moveToKeyGreaterThan(const std::string& key, 
    ...
    for (level_t level = 0; level < height_; level++) {
        ...
        bool has_child = child_indicator_bitmaps_->readBit(pos);
        if(!has_child && level == key.length()-1) {
            iter.setFlags(true, true, true, true);
            return true;
        }
        if(!has_child) {
...
int LoudsDense::Iter::compare(const std::string& key) const {
    ...
    int cmp = iter_key.compare(key_dense);
    if (cmp != 0) return cmp;
    if (isComplete()) {
...
int LoudsDense::Iter::getValue(word_t* val) const {
    if (isComplete()) {
        position_t value_pos = trie_->getSuffixPos(pos_in_trie_[key_len_ - 1], is_at_prefix_key_);
        *val = trie_->values_->readReal(value_pos);
    return trie_->values_->getRealSuffixLen();
    }
    *val = 0;
    return 0;
}
